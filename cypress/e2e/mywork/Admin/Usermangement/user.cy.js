...
describe("Validating User", () => {
  ...
  it.only("Validating filter of system user", () => {
    cy.login("Admin", "admin123");
    cy.admin();
    adminUserObj.filterButton.should("be.visible");
    adminUserObj.filterTabClick.click();
    adminUserObj.filterButton.should("not.be.visible");
  });
  ...
  it.only("Validating Userrole dropdown block", () => {
    cy.login("Admin", "admin123");
    cy.admin();
    adminUserObj.userRole.find("div").find("i").eq(0).click();
  ...
  it("Validating Employee Name block", () => {
    ...
    adminUserObj.employeeRoleDropdown
      ...
      .find("span")
      .then((resp) => {
        cy.wrap(resp).contains("Krishna Kumar Pasupuleti").click();
      });
  });
  it.only("Validating Status box", () => {
    cy.login("Admin", "admin123");
    cy.admin();
    adminUserObj.status.find("div").find("i").eq(1).click();
